...
def _rerun_app():
    """Tüm uygulamayı yeniden çalıştırır — dialog/fragment içinden de güvenli.

    st.rerun() fragment (dolayısıyla dialog) içinden çağrıldığında kapsamın
    'app' olması gerekir; aksi hâlde yalnız fragment yenilenir ve dialog
    kapanmaz. scope parametresi Streamlit 1.37'de geldi, eski sürümlerde yok —
    bu yüzden önce scope'lu deneriz, desteklenmiyorsa sade çağrıya düşeriz.
    """
    try:
        st.rerun(scope="app")
    except TypeError:
