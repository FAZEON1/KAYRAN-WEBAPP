...
def _urun_karlilik_bolumu():
    ...
    st.markdown('<div style="display:inline-block;padding:6px 14px;background:rgba(99,102,241,0.12);border:1px solid rgba(99,102,241,0.25);border-radius:20px;margin-bottom:18px"><span style="color:#A5B4FC;font-size:11px;font-weight:600;letter-spacing:1px;text-transform:uppercase">🧮 Urün Karlilik Hesaplayici</span></div>', unsafe_allow_html=True)
    ...
    with col1:
        urun_adi = st.text_input("Urun Adi / Kodu (opsiyonel)", placeholder="Örn: Monitor XG27AQM", key="hm_urun_adi")
    with col2:
        alis_fiyati = st.number_input("Alis Fiyati ($)", min_value=0.0, value=0.0, step=0.01, format="%.2f", key="hm_alis")
    with col3:
        masraflar = st.number_input("Ek Masraflar ($)", min_value=0.0, value=0.0, step=0.01, format="%.2f", help="Kargo, komisyon, ambalaj vb.", key="hm_masraf")
    ...
    with colm2:
        st.markdown('<div class="hm-label">Ek Masraflar</div><div class="hm-deger">$' + "{:.2f}".format(masraflar) + '</div>', unsafe_allow_html=True)
    with colm3:
    ...
    for idx, sen in enumerate(st.session_state.hm_senaryolar):
        ...
        with cs5:
            sil_btn = st.button("🗑️", key="sen_sil_" + str(sen_id), help="Senaryoyu sil")
    ...
    if kaydet_btn:
        if toplam_maliyet <= 0:
            st.warning("⚠️ Lutfen once maliyet bilgilerini girin.")
        elif not senaryolar_sonuc:
            st.warning("⚠️ En az bir senaryo eklemelisiniz.")
        else:
            ok, msg = _hesap_kaydet(aktif_kullanici, urun_adi, alis_fiyati, masraflar, senaryolar_sonuc, notlar_kayit)
            if ok:
...
def _gecmis_hesaplamalar():
    ...
    headers = ["Urun / Kod", "Alis ($)", "Masraflar ($)", "Top. Maliyet ($)", "Tarih", "Islem"]
    th1, th2, th3, th4, th5, th6 = st.columns([2, 1.2, 1.2, 1.2, 1.5, 1])
    for col, h in zip([th1, th2, th3, th4, th5, th6], headers):
        with col:
    ...
    for kayit in kayitlar:
        ...
        with rc3:
            st.markdown('<div style="color:#CBD5E1;font-size:13px;padding:8px 0">$' + "{:.2f}".format(mas) + '</div>', unsafe_allow_html=True)
        with rc4:
    ...
    with dp1:
        if st.button("💾 Notu Kaydet", key="hm_not_kaydet_" + str(rid)):
            if _hesap_guncelle(rid, yeni_not):
                ...
                st.rerun()
    with dp2:
        if st.button("🗑️ Kaydi Sil", key="hm_kayit_sil_" + str(rid), type="primary"):
            st.session_state["hm_sil_onay_" + str(rid)] = True
    if st.session_state.get("hm_sil_onay_" + str(rid), False):
        st.warning("⚠️ Bu kaydi kalici olarak silmek istiyor musunuz?")
...
def _kayitlari_excel_olustur(kayitlar):
    try:
        ...
        for k in kayitlar:
            lines.append(",".join(str(x) for x in [k.get("urun_adi",""), k.get("alis_fiyati",0),
                k.get("masraflar",0), k.get("toplam_maliyet",0), k.get("olusturma_tarihi",""), k.get("notlar","")]))
    ...
    headers = ["Urun Adi/Kodu", "Alis Fiyati ($)", "Ek Masraflar ($)", "Toplam Maliyet ($)",
    ...
    for kayit in kayitlar:
        ...
        for sen in sens:
            ...
            row_data = [kayit.get("urun_adi",""), kayit.get("alis_fiyati",0), kayit.get("masraflar",0),
                toplam_maliyet, sen.get("ad",""), round(sf,2), round(inf,2), round(kar,2),
                round(marj or 0, 2), tar, kayit.get("notlar","")]
