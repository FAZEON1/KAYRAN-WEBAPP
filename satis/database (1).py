...
# ── 🧭 GEÇMİŞ REVİZYONU — fatura dökümü tek doğruluk kaynağı ─────────
# Yalnız `satislar` tablosuna dokunur; stok etkisi simetriktir (silinen
# satış stoğu geri verir, eklenen düşer). İadeler, alınan destekler,
# kampanyalar, ithalat, muhasebe, ürünler: HİÇBİRİNE DOKUNULMAZ.

def _revizyon_aralik_kayitlari(araliklar):
    """Verilen tarih aralıklarının ([('2025-01-01','2025-12-31'), ...])
    kapsadığı mevcut satış satırlarını (id bazında, tekilleştirilmiş) döndürür."""
    cli = _get_client()
    kayitlar = {}
    for bas, bit in (araliklar or []):
        if not bas or not bit:
            continue
        rows = _rows(cli.table("satislar")
                     .select("id, sku, adet, birim_satis, "
                             "kampanya_id, birim_firma_destek, birim_ek_destek, "
                             "siparis_no, kanal, tarih")
                     .gte("tarih", str(bas)[:10]).lte("tarih", str(bit)[:10])
                     .order("id").execute())
        for r in rows:
            kayitlar[r["id"]] = r
...
def _revizyon_korunan_mi(r, kampanya_koru):
    """Kampanya bağlantılı satır mı? (kampanya_id dolu ya da destek işlenmiş)"""
    if not kampanya_koru:
        return False
    return bool(r.get("kampanya_id")) or _f(r.get("birim_firma_destek")) > 0 \
        or _f(r.get("birim_ek_destek")) > 0


def satis_revizyon_onizle(araliklar, kampanya_koru=True):
    """Hiçbir şey yazmadan: kapsanan dönemdeki mevcut kayıtların özeti.
    Döner: {satir, adet, ciro, silinecek, silinecek_adet, silinecek_ciro,
            korunan, korunan_adet, korunan_ornek}"""
    try:
        rows = _revizyon_aralik_kayitlari(araliklar)
        o = {"satir": len(rows), "adet": 0, "ciro": 0.0,
             "silinecek": 0, "silinecek_adet": 0, "silinecek_ciro": 0.0,
             "korunan": 0, "korunan_adet": 0, "korunan_ornek": []}
        for r in rows:
            _a = _i(r.get("adet"))
            _c = _a * _f(r.get("birim_satis"))
            o["adet"] += _a
            o["ciro"] += _c
            if _revizyon_korunan_mi(r, kampanya_koru):
                o["korunan"] += 1
                o["korunan_adet"] += _a
                if len(o["korunan_ornek"]) < 5:
                    o["korunan_ornek"].append(
                        f"{r.get('siparis_no') or '—'} · {r.get('sku') or ''} · {_a} adet")
            else:
                o["silinecek"] += 1
                o["silinecek_adet"] += _a
                o["silinecek_ciro"] += _c
        ...
        return {"hata": f"{type(e).__name__}: {str(e)[:140]}"}


def satis_revizyon_uygula(satirlar, araliklar, kampanya_koru=True, ilerleme=None):
    """REVİZYON: aralıktaki (korunanlar hariç) TÜM satışları siler, sonra
    dosya satırlarını ekler. Stok iki yönde de simetrik uygulanır.
    Döner: ice_aktar sonucu + {'silinen_satir', 'korunan_satir'}."""
    try:
        cli = _get_client()
        rows = _revizyon_aralik_kayitlari(araliklar)
        sil_rows = [r for r in rows if not _revizyon_korunan_mi(r, kampanya_koru)]
        korunan = len(rows) - len(sil_rows)

        # 1) Sil (id bazında, 100'lük gruplar) + stoğu geri ver
        sil_ids = [r["id"] for r in sil_rows]
        for i in range(0, len(sil_ids), 100):
            cli.table("satislar").delete().in_("id", sil_ids[i:i + 100]).execute()
        if sil_rows:
            _stok_uygula(_satis_agg(sil_rows), +1, "satis_revizyon_sil")

        # 2) Dosyayı ekle — aralık boşaltıldı; atla_mevcut kapalı (hepsi girsin).
        #    Maliyet paçaldan, SKU normalize, stok düşümü ice_aktar içinde.
        sonuc = ice_aktar_satislar(satirlar, atla_mevcut=False,
                                   temizle_once=False, ilerleme=ilerleme)
        sonuc["silinen_satir"] = len(sil_ids)
        sonuc["korunan_satir"] = korunan
        _temizle()
        return sonuc
    except Exception as e:
        return {"eklendi": 0, "atlandi": 0, "maliyetsiz": 0, "hatali": 0,
                "silinen_satir": 0, "korunan_satir": 0,
