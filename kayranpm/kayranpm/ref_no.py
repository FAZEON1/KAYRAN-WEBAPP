...
def kategori_dagit_dialog(r, firma_adi=""):
    ...
    """
    import json as _json
    # "TÜM KATEGORİLERE EŞİT BÖL" bir kategori DEĞİL, bir talimattır. Kutu
    # açmak kullanıcıyı kilitliyordu: oraya ne yazacağı belirsiz olduğu için
    # kalan hiç sıfırlanmıyor, tam eşitlik şartı da Kaydet'i kapalı tutuyordu.
    _ESIT_ETIKET = ("TÜM KATEGORİLERE EŞİT BÖL", "TUM KATEGORILERE ESIT BOL",
                    "TÜM KATEGORİLER", "TUM KATEGORILER")
    _tum_kats = _kat_liste(r.get("kategori"))
    _kats = [k for k in _tum_kats if k not in _ESIT_ETIKET]
    _esit_var = len(_kats) != len(_tum_kats)
    if not _kats:
        _kats = _tum_kats
    _ham = round(_f(r.get("tutar")), 2)      # 99903.78000000003 gibi artıkları temizle
    ...
    if _esit_var:
        st.caption("ℹ️ Excel'de **tüm kategorilere eşit böl** yazan satırlar var. "
                   "Onlar için kutu açılmadı — aşağıdaki **⚡ eşit böl** düğmesini kullan.")
    ...
    _top = round(sum(_giris.values()), 2)
    _kalan = round(_ham - _top, 2)
    if abs(_kalan) < 0.005:
        st.success(f"✓ Dağıtım tam: {_sm}{_top:,.2f}")
    elif _top == 0:
        st.info("Henüz dağıtılmadı — tamamı **GENEL**'de kalır ve kategori "
                "filtresine yansımaz.")
    else:
        st.warning(f"⚠️ Dağıtılmayan: {_sm}{_kalan:,.2f} — bu tutar **GENEL**'de "
                   f"kalır, Kâr/P&L merdiveninde 'dağıtılmamış' olarak görünür. "
                   f"Kaydetmene engel değil.")

    # Yardımcı: kalanı kategorilere eşit böl. "TÜM KATEGORİLERE EŞİT BÖL"
    # etiketi bunun için var; artık kutu değil düğme olarak sunuluyor.
    if abs(_kalan) > 0.005 and _kats:
        _et = ("⚡ Kalanı kategorilere eşit böl"
               + (" (Excel'de 'tüm kategorilere eşit böl' yazıyor)" if _esit_var else ""))
        if st.button(_et, use_container_width=True, key=f"refkt_esit_{r.get('id')}"):
            _pay = round(_kalan / len(_kats), 2)
            _dag = 0.0
            for _i2, _k2 in enumerate(_kats):
                _v2 = _pay if _i2 < len(_kats) - 1 else round(_kalan - _dag, 2)
                st.session_state[f"refkt_{r.get('id')}_{_k2}"] = float(
                    round(_giris.get(_k2, 0) + _v2, 2))
                _dag = round(_dag + _v2, 2)
            st.rerun()

    _c1, _c2 = st.columns(2)
    # Kaydet ARTIK KİLİTLENMİYOR — kısmi dağıtım geçerli bir durumdur.
