...
def run():
 ...
 <div style="margin-top:7px;"><span style="display:inline-block;padding:3px 11px;border-radius:7px;background:rgba(99,102,241,0.15);border:1px solid rgba(99,102,241,0.25);color:#A5B4FC;font-family:'JetBrains Mono',monospace;font-size:11px;font-weight:600;letter-spacing:0.5px;">{secilen["sku"]}</span></div>
    ...
    elif sayfa == "🏬  Depo Yönetimi":
        st.markdown('<div class="baslik">🏬 Depo Yönetimi</div>', unsafe_allow_html=True)
        st.markdown('<div class="alt-baslik">Depo bazlı stok · depolar arası sevk · canlı adet revizyonu</div>', unsafe_allow_html=True)
        st.markdown('<div class="sayfa-baslik-cizgi"></div>', unsafe_allow_html=True)
        from .database import (get_depo_ozet, get_depo_listesi, get_depo_stok,
                               depo_sevk, get_depo_sevk_gecmisi)

        _ozet = get_depo_ozet()
        if not _ozet:
            st.info("Henüz depo bazlı stok yok. 'Veri Yükleme' → **G5F Stok (Depo Kırılımlı)** "
                    "Excel'ini yükleyince depolar burada görünür.")
        else:
