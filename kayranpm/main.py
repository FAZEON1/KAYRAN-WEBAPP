...
def run():
    ...
    # ════════════════════════════════════════════════════════════════════
        ...
        if df_oz.empty:
            ...
            st.caption("✏️ **Ürün Adı, Kategori, Satış ($) ve G5F Depo** hücrelerine çift tıklayıp doğrudan düzenleyebilirsiniz. Diğer sütunlar otomatik hesaplanır. Düzenleyince aşağıdan **Değişiklikleri Kaydet**'e basın.")
            edited = st.data_editor(
                df_oz,
                use_container_width=True,
                height=520,
                hide_index=True,
                disabled=["SKU", "ITOPYA", "HB", "VATAN", "MONDAY", "KANAL", "Toplam", "FOB ($)", "Final Cost ($)", "Net Kar ($)", "Kamp."],
                key="urun_editor",
                column_config={
