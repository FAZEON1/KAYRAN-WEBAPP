...
def talep_gonder(gonderen_ad, konu, mesaj):
    ...
    html = (
        "<div style='font-family:Arial,Helvetica,sans-serif;font-size:15px;color:#0F1730;line-height:1.6'>"
...
if not st.session_state.get("_db_saglik_ok"):
    ...
    if _ok:
        ...
        st.markdown(
            ...
            'background:#232B47;border:1px solid #232B47;border-radius:14px;'
...
def _kayran_plotly_tema():
    ...
    _pio.templates["kayran"] = _pgo.layout.Template(layout=dict(
        ...
        hoverlabel=dict(bgcolor="#131B38", bordercolor="rgba(129,140,248,0.4)",
...
(function () {
  ...
  btn.style.cssText = [
    ...
    'background:#0F1730','color:#F59E0B',
...
# ── RENK KONSOLİDASYONU (çalışma anında, TEK NOKTADAN) ─────────────────────
# Arayüz genelinde 163 farklı hex birikmişti. Modül dosyalarına dokunmak
# yerine, sayfa çizilirken eski tonları kanonik palete çeviren bir gözlemci
# kuruldu: 75 yakın-kopya ton 17 çekirdek renkte birleşir. Anlam taşıyan
# renkler (durum çipleri, yeşil/kırmızı uyarı zeminleri #0A2D15/#2D0A0A,
# açık mavi rozetler #BFDBFE) bilerek LİSTEDE YOK — olduğu gibi kalır.
# Kaldırmak istersen bu bloğu silmen yeter; modüller orijinal renklerine döner.
_sb_comp.html(
    """
<script>
(function () {
  const w = window.parent, doc = w.document;
  if (w.__kayranPalet) return;
  w.__kayranPalet = true;

  // eski ton → kanonik token (shared/tema.py paletiyle aynı)
  const M = {
    "FFFFFF": "F1F5F9",   // saf beyaz → yumuşak beyaz (parlama biter)
    "0F172A": "0F1730", "152036": "131B38", "131C35": "131B38",
    "0E1A3A": "131B38", "151F38": "131B38", "1E293B": "232B47",
    "1A2540": "232B47",                       // eski panel/çerçeve → yeni zemin dili
    "8B97A8": "94A3B8", "78909C": "64748B",   // dağınık griler → tek ton
    "93C5FD": "A5B4FC", "90CAF9": "A5B4FC", "C4B5FD": "A5B4FC",
    "8B5CF6": "6366F1", "42A5F5": "38BDF8",   // mavi/mor sapmalar → marka ailesi
    "4ADE80": "34D399", "FCA5A5": "FDA4AF"    // yeşil/kırmızı sapmalar → tek ton
  };
  const RX = new RegExp("#(?:" + Object.keys(M).join("|") + ")", "gi");
  const cevir = s => s.replace(RX, m => "#" + M[m.slice(1).toUpperCase()]);
  ...
  function isle(el) {
    ...
    if (el.tagName === "STYLE") {
      const y = cevir(el.textContent);
      if (y !== el.textContent) el.textContent = y;
    }
  }
  function tara(kok) {
    isle(kok);
    if (kok.querySelectorAll)
      kok.querySelectorAll("[style],style").forEach(isle);
  }
  tara(doc.body);
  new MutationObserver(function (muts) {
    for (const m of muts) {
      if (m.type === "attributes") isle(m.target);
      if (m.addedNodes)
        m.addedNodes.forEach(function (n) { if (n.nodeType === 1) tara(n); });
    }
...
# KURUMSAL KIMLIK
# ─────────────────────────────────────────────────────────────────────
KAYRAN_LOGO_SVG = '<svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg"><defs><linearGradient id="kgS" x1="0" y1="0" x2="40" y2="40" gradientUnits="userSpaceOnUse"><stop offset="0%" stop-color="#6366F1"/><stop offset="50%" stop-color="#7C3AED"/><stop offset="100%" stop-color="#2563EB"/></linearGradient></defs><rect width="40" height="40" rx="10" fill="url(#kgS)"/><polygon points="9,8 15,8 15,19 24,8 32,8 21,21 32,32 24,32 15,21 15,32 9,32" fill="white"/></svg>'

KAYRAN_LOGO_BIG = '<svg width="64" height="64" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg"><defs><linearGradient id="kgB" x1="0" y1="0" x2="64" y2="64" gradientUnits="userSpaceOnUse"><stop offset="0%" stop-color="#6366F1"/><stop offset="50%" stop-color="#7C3AED"/><stop offset="100%" stop-color="#2563EB"/></linearGradient></defs><rect width="64" height="64" rx="16" fill="url(#kgB)"/><polygon points="14,13 24,13 24,30 38,13 51,13 34,34 51,51 38,51 24,34 24,51 14,51" fill="white"/></svg>'
...
def login_css():
    ...
    /* Default rengi koyu gri (#232B47) — koyu zeminde okunmaz, beyaza çeviriyoruz */
...
def portal_css():
    ...
    section[data-testid="stSidebar"] {
        background: linear-gradient(180deg, #0F1730 0%, #080C20 100%) !important;
    ...
    /* ═══════════ TEMA RÖTUŞU — tipografi & tutarlılık katmanı ═══════════
       Amaç: tek dosyadan tüm modüllere yansıyan sessiz iyileştirmeler.
       Modül kodlarına dokunulmaz; inline stiller olduğu gibi çalışır. */

    /* 1) Başlık hiyerarşisi — sıkı izleme (tracking), net ölçek */
    ...
    .main h4 { font-weight: 700 !important; letter-spacing: -0.01em !important; }

    /* 2) Rakam hizası — finans ekranlarında rakamlar alt alta hizalanır
       (tabular-nums; HTML tablolar, metrikler ve markdown için) */
    .main table, .main [data-testid="stMetric"], .main .stMarkdown,
    .main [data-testid="stMetricValue"] {
        font-variant-numeric: tabular-nums !important;
        font-feature-settings: "tnum" 1 !important;
    }

    /* 3) HTML tablo okunabilirliği — satır ayrımı fare ile takip edilir */
    .main .stHtml table tbody tr { transition: background .12s ease; }
    .main .stHtml table tbody tr:hover { background: rgba(99,102,241,0.07) !important; }
    .main .stHtml table thead th { font-weight: 700 !important; }

    /* 4) Saf beyaz parlamasını yumuşat — seçim ve kalın metin tonu */
    .main strong, .main b { color: #F1F5F9; }
    ::selection { background: rgba(99,102,241,0.35); color: #F1F5F9; }

    /* 5) Girdi odak halkası — her modülde aynı, erişilebilir kontrast */
    ...
    /* 6) Expander / dialog / popover — köşe ve çerçeve dili tek tip */
    .main [data-testid="stExpander"] {
        border: 1px solid rgba(255,255,255,0.07) !important;
        border-radius: 12px !important;
        background: rgba(255,255,255,0.015) !important;
    }
    .main [data-testid="stExpander"] summary { font-weight: 600 !important; }
    div[data-testid="stDialog"] > div { border-radius: 16px !important; }

    /* 7) Metrik kartları — modüller arası aynı zemin dili */
    .main [data-testid="stMetric"] {
        background: linear-gradient(180deg, #131B38 0%, #0F1730 100%);
    ...
    /* 8) Ayırıcılar ve kaydırma çubuğu — görsel gürültüyü azalt */
    .main hr { border-color: rgba(255,255,255,0.07) !important; }
    ::-webkit-scrollbar { width: 9px; height: 9px; }
    ::-webkit-scrollbar-thumb { background: #232B47; border-radius: 6px; }
    ::-webkit-scrollbar-thumb:hover { background: #232B47; }
    ::-webkit-scrollbar-track { background: transparent; }

    /* 9) Caption/soluk metin — tek gri ton (rastgele griler yerine) */
    .main [data-testid="stCaptionContainer"], .main small {
...
@media (max-width: 640px) {
    ...
    with col_l:
        st.markdown(
            ...
            f'{KAYRAN_LOGO_BIG}'
            '<div>'
            '<div style="font-family:Inter,sans-serif;font-size:23px;font-weight:900;color:#F1F5F9;letter-spacing:5px;line-height:1">KAYRAN</div>'
            ...
            '<h2 style="font-family:Inter,sans-serif;font-size:23px;font-weight:700;color:#F1F5F9;line-height:1.3;margin:0 0 8px">'
    ...
    with col_r:
        with st.container(border=True):
            st.markdown(
                ...
                '<div style="color:#F1F5F9;font-size:19px;font-weight:700;margin-bottom:8px">Hesabınıza Giriş Yapın</div>'
...
def ust_navigasyon():
    ...
    .st-key-ustnav button[kind="primary"]{
        ...
        color:#F1F5F9 !important;box-shadow:0 2px 10px rgba(79,70,229,0.35) !important;}
    ...
    [data-testid="stMainBlockContainer"] div[role="radiogroup"] > label:has(input:checked) p{color:#F1F5F9 !important;font-weight:700 !important;}
...
input, textarea, select { font-size: 16px !important; }
    ...
    st.markdown(
        ...
        '}'
        '::-webkit-scrollbar{width:10px;height:10px;}'
        '::-webkit-scrollbar-track{background:linear-gradient(180deg,#131B38,#0F1730);}'
    ...
    with st.sidebar:
        ...
        st.markdown(
            ...
            '<span style="font-family:Inter,sans-serif;font-size:16px;font-weight:800;color:#F1F5F9;letter-spacing:1.5px;line-height:1">KAYRAN</span>'
            '<span style="font-size:9.5px;color:#64748B;letter-spacing:1.2px;text-transform:uppercase;font-weight:600">Workspace</span>'
            '</div>'
            '</div>',
            unsafe_allow_html=True
        )
        ...
        _lh = ('<details style="margin:0 0 10px"><summary style="cursor:pointer;color:#64748B;'
        ...
        for _ad, _mod in _yeni_sekme:
            _lh += (f'<a href="?t={_tok_aktif}&s={_mod}" target="_blank" '
                    f'style="display:block;padding:8px 12px;background:linear-gradient(180deg,#131B38,#0F1730);'
        ...
        if aktif_sayfa in ("anasayfa", "kayrantsw", "sifre_degistir", "hesap_makinesi"):
            ...
            st.markdown(
                '<div style="display:flex;align-items:center;gap:8px;padding:8px 12px;background:linear-gradient(180deg,#131B38,#0F1730);border:1px solid rgba(255,255,255,0.06);border-radius:10px;margin-bottom:8px">'
                '<div style="width:30px;height:30px;border-radius:8px;background:linear-gradient(135deg,#6366F1,#6366F1);display:flex;align-items:center;justify-content:center;font-weight:700;color:white;font-size:13px">' + ilk_harf + '</div>'
                ...
                '<div style="color:#F1F5F9;font-weight:600;font-size:13px;margin-top:0px;line-height:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' + aktif_kullanici.capitalize() + '</div>'
                '</div>'
                '</div>',
                unsafe_allow_html=True
            )
            ...
            uyg_renk_map = {"kayranacc": "#A5B4FC", "kayranpm": "#FDA4AF", "depo": "#6EE7B7", "ithalat": "#7DD3FC", "teknikservis": "#FDA4AF", "hesap_makinesi": "#FCD34D"}
...
def anasayfa():
    G5F_LOGO_SVG = '<svg width="100" height="44" viewBox="0 0 220 90" fill="none" xmlns="http://www.w3.org/2000/svg" style="display:inline-block"><text x="10" y="72" font-family="Inter, sans-serif" font-size="80" font-weight="900" fill="#F1F5F9">G</text><text x="78" y="72" font-family="Inter, sans-serif" font-size="80" font-weight="900" fill="#FB923C">5</text><text x="142" y="72" font-family="Inter, sans-serif" font-size="80" font-weight="900" fill="#F1F5F9">F</text></svg>'
    FAZEON_LOGO_SVG = '<svg width="170" height="32" viewBox="0 0 360 60" fill="none" xmlns="http://www.w3.org/2000/svg" style="display:inline-block"><text x="0" y="44" font-family="Inter, sans-serif" font-size="44" font-weight="300" fill="#F1F5F9" letter-spacing="6">FAZEON</text></svg>'
    aktif_kullanici = st.session_state.get("aktif_kullanici", "")
    yetkiler = kullanici_yetkileri(aktif_kullanici)
    ...
    st.markdown(
        ...
        '</div>'
        f'<h1 style="font-family:Inter,sans-serif;font-size:clamp(26px,5vw,40px);font-weight:800;color:#F1F5F9;letter-spacing:-0.5px;line-height:1.1;margin:0">'
        f'{selamlama}, '
    ...
    def _g_card(ust, buyuk, alt, accent, ikon):
        return (f'<div style="background:rgba(255,255,255,0.04);border:1px solid {accent}2e;border-radius:16px;'
                ...
                f'<div style="color:#F1F5F9;font-size:23px;font-weight:800;line-height:1;font-family:JetBrains Mono,monospace">{buyuk}</div>'
    ...
    def _kpi_card(label, value, sub, accent):
        return (f'<div style="background:linear-gradient(180deg,#131B38,#0F1730);border:1px solid {accent}33;border-radius:12px;'
                ...
                f'<div style="color:#F1F5F9;font-size:19px;font-weight:800;font-family:JetBrains Mono,monospace;line-height:1">{value}</div>'
    ...
    with st.expander("🏢 Kurumsal · G5F Teknoloji & Fazeon", expanded=False):
        ...
        with _bk1:
            st.markdown(
                '<div style="background:linear-gradient(135deg,#131B38 0%,#0F1730 100%);'
                'border:1px solid rgba(232,132,32,0.2);border-left:3px solid #FB923C;border-radius:14px;padding:16px 20px;display:flex;flex-direction:column;min-height:200px">'
                f'<div style="height:46px;display:flex;align-items:center;margin-bottom:8px">{G5F_LOGO_SVG}</div>'
                '<div style="font-size:15px;font-weight:800;color:#F1F5F9;margin-bottom:0px">G5F Teknoloji</div>'
        ...
        with _bk2:
            st.markdown(
                '<div style="background:linear-gradient(135deg,#080C20 0%,#131B38 50%,#080C20 100%);'
                ...
                '<div style="font-size:15px;font-weight:800;color:#F1F5F9;margin-bottom:0px">Fazeon</div>'
    ...
    st.markdown(
        ...
        'border-color:#6366F1 !important;box-shadow:0 0 0 3px rgba(139,92,246,0.15) !important;}'
        '</style>',
        unsafe_allow_html=True
    )
    ...
    st.markdown(
        ...
        '<div style="display:inline-flex;align-items:center;gap:16px;padding:8px 16px;background:linear-gradient(180deg,#131B38,#0F1730);border:1px solid rgba(255,255,255,0.04);border-radius:30px">'
            ...
            if not online_listesi:
                ...
                for u in online_listesi:
                    ...
                    cards_html += (
                        ...
                        f'<div style="overflow:hidden"><div style="color:#F1F5F9;font-weight:600;font-size:13px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">{k_adi.capitalize()}</div>'
...
def sifre_degistir():
    ...
    st.markdown(
        ...
        '</div>'
        '<h1 style="font-family:Inter,sans-serif;font-size:clamp(24px,5vw,36px);font-weight:800;color:#F1F5F9;margin:0">Şifremi Değiştir</h1>'
    ...
    st.markdown(
        ...
        'border:1px solid rgba(255,255,255,0.12) !important;color:#F1F5F9 !important;border-radius:12px !important;}'
        '[data-testid="stTextInput"] input:focus{border-color:#6366F1 !important;'
        'box-shadow:0 0 0 3px rgba(139,92,246,0.15) !important;}'
        '.stFormSubmitButton>button{background:linear-gradient(135deg,#6366F1,#6366F1) !important;'
    ...
    with col_c:
        st.markdown(
            '<div style="background:linear-gradient(180deg,#131B38,#0F1730);border:1px solid rgba(255,255,255,0.08);'
            ...
            f'<div style="width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,#6366F1,#6366F1);'
            f'display:flex;align-items:center;justify-content:center;font-weight:700;color:white;font-size:15px">{ilk_harf}</div>'
            f'<div><div style="color:#F1F5F9;font-weight:600;font-size:15px">{aktif_kullanici.capitalize()}</div>'
...
def kayrantsw_yakinda():
    ...
    st.markdown(
        ...
        '</div>'
        # Başlık
        '<h1 style="font-family:Inter,sans-serif;font-size:clamp(26px,5vw,44px);font-weight:800;color:#F1F5F9;'
...
def _global_hata_kart(uygulama_adi, hata):
    st.markdown(
        ...
        f'<b style="color:#FDA4AF;font-size:19px">{uygulama_adi} Uygulamasında Bir Sorun Oluştu</b>'
        '</div>'
        ...
        '</div>'
        '<div style="background:rgba(0,0,0,0.25);border:1px solid rgba(248,113,113,0.25);border-radius:8px;padding:12px 16px;font-family:monospace;font-size:13px;color:#FDA4AF;margin-bottom:16px;overflow-x:auto">'
